...
const UseLoadingHook = (selectAPI:string, searchTerm:SearchParams, refreshTerm?:string)=>{
    ...
    useEffect(()=>{
        setIsLoading(true)
            if ('pSize' in searchTerm && 'p' in searchTerm && 'q' in searchTerm && 'museum' in searchTerm && 'after' in searchTerm && 'before' in searchTerm&&(searchTerm.q||searchTerm.museum||searchTerm.after||searchTerm.before)){
                if (selectAPI==="SMG"){
